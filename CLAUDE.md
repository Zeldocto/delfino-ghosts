...
6. **MDP is always derived**, never stored as a flag. It is `SUM(download_count)` per author,
...
| `0x78` | Run label, 48 bytes, e.g. `Bianco Hills 3 - 0:37.337` (**inferred**, one sample) |
...
- Schema changes ship as a new numbered migration, and `0001_init.sql` is updated in step so a fresh
  install matches an upgraded one. Say clearly which file the owner needs to run.
